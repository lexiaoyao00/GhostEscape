...
HUDText *HUDText::addHUDTextChild(Object *parent, const std::string &text, glm::vec2 render_pos, glm::vec2 size,
    const std::string &font_path, int font_size, const std::string &bg_path, Anchor anchor)
{
...
{
    auto text_size = text_labe_->getSize();
...
void HUDText::setBackground(const std::string &file_path)
{
    ...
    {
        sprite_bg_ = Sprite::addSpriteChild(this, file_path);
    }
